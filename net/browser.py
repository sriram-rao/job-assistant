...
class Browser:
    ...
    def go_to(
        ...
        *,
        ...
        _ = self.page.goto(url, wait_until=wait_until, timeout=timeout_ms)
    ...
    def find_link_by_text(self, link_text: str, *, exact: bool = False) -> Locator:
        return (
    ...
    def click_link_with_text(
        self, link_text: str, *, exact: bool = False, timeout_ms: int = 30_000
    ) -> str:
    ...
    def extract_labels(self) -> list[str]:
        return [
            t.strip()
            for t in self.page.locator("label").all_text_contents()
            if t.strip()
        ]

    def extract_inputs(self, *, timeout_ms: int = 30_000) -> list[dict[str, object]]:
    ...
    def build_payload_from_url(
        ...
        *,
        ...
        if not submit["submit_buttons"] and self.get_link_containing("Apply"):
            self.go_to(
                self.get_link_containing("Apply") or url,
        ...
        if not submit["submit_buttons"]:
            ...
            submit: dict[str, object] = {
                **parse_submit_hints_from_html(html, base_url=self.page.url),
                **{"submit_buttons": {}},
            }
        # Convert fields to list[dict[str, str]] for build_payload
        str_fields: list[dict[str, str]] = []
        for f in fields:
            str_field = {k: str(v) if v is not None else "" for k, v in f.items()}
            str_fields.append(str_field)
        data, files = build_payload(str_fields)
