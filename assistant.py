...
class Assistant:
    ...
    def store_application(self, application: dict[str, Any], out_dir: Path | None = None) -> dict[str, Path]:
        ...
        generate_letter_files(details, application["letter"])
        generate_resume_files(application["work_experience"], application["skills"])
        ...
        letter_pdf_final, resume_pdf_final = rename_pdfs(target_dir, details.get("company", ""))
    ...
    def ask_about(self, question: str, about_url: str) -> str:
        return self.ask(self.get_context(about_url) + "\n" + question, model='gpt-5-mini', temperature=1, max_tokens=3000)
